...
static u64 guest_init_efer;

static void vmx_exit_control_efer_test_guest(void)
{
	guest_init_efer = rdmsr(MSR_EFER);
...
static void vmx_exit_control_efer_test(void)
{
	...
	/* Allow the guest to read the EFER directly */
	msr_bmp_init();
	...
	enter_guest();
	report(guest_init_efer == GUEST_INIT_EFER, "Guest EFER was set to its initial value at VM entry");
	report(rdmsr(MSR_EFER) == GUEST_SET_EFER, "The EFER MSR is retained at VM Exit");
	report(vmcs_read(GUEST_EFER) == GUEST_INIT_EFER, "VMCS guest EFER is not saved at VM Exit 0x%lx", vmcs_read(GUEST_EFER));
	report(vmcs_read(HOST_EFER) == host_efer &&
	       vmcs_read(HOST_EFER) != rdmsr(MSR_EFER),
	       "VMCS host EFER is not loaded at VM Exit");
	...
	report(vmcs_read(HOST_EFER) == host_efer &&
	       vmcs_read(HOST_EFER) == rdmsr(MSR_EFER),
	       "VMCS host EFER is loaded at VM Exit");
