...
static void vmx_exit_control_fred_test(void)
{
	 ...
	 * Case 1:
	 *
	 * FRED MSRs are retained if the "Load FRED" VM-Exit control is cleared.
	 *
	 * FRED MSRs are not saved to VMCS during VM exit if the "Save FRED"
	 * VM-Exit control is cleared.
	 */
	 ...
	 * Case 2:
	 *
	 * FRED MSRs are loaded with values from VMCS host FRED MSR fields
	 * if the "Load FRED" VM-Exit control is set.
	 *
	 * FRED MSRs are not saved to VMCS during VM exit if the "Save FRED"
	 * VM-Exit control is cleared.
	 */
	 ...
	 * FRED MSRs are retained if the "Load FRED" VM-Exit control is cleared.
	 *
	 * FRED MSRs are saved to VMCS during VM exit if the "Save FRED"
	 * VM-Exit control is set.
	 */
...
test_again:
	 ...
	 * Case 1:
	 *
	 * The EFER MSR is not saved to VMCS during VM-Exit if the "Save EFER"
	 * VM-Exit control is cleared.
	 *
	 * The EFER MSR is retained if the "Load EFER" VM-Exit control is
	 * cleared.
	 */
	 ...
	 * VM-Exit control is cleared.
	 *
	 * The EFER MSR is loaded with the value from VMCS host EFER MSR field
	 * if the "Load EFER" VM-Exit control is set.
	 */
	 ...
	 * Case 3:
	 *
	 * The EFER MSR is saved to VMCS during VM-Exit if the "Save EFER"
	 * VM-Exit control is set.
	 *
	 * The EFER MSR is retained if the "Load EFER" VM-Exit control is
	 * cleared.
	...
	if (!entry_not_load_efer_tested) {
		...
		/* Clear ENT_LOAD_EFER and run tests again */
...
{
	bool entry_not_load_pat_tested = false;
	u64 host_pat = rdmsr(MSR_IA32_CR_PAT);
	...
	vmcs_write(HOST_PAT, host_pat);

test_again:
	/*
	 * Case 1:
	 *
	 * The PAT MSR is not saved to VMCS during VM-Exit if the "Save PAT"
	 * VM-Exit control is cleared.
	 *
	 * The PAT MSR is retained if the "Load PAT" VM-Exit control is cleared.
	 */
	vmcs_clear_bits(EXI_CONTROLS, EXI_SAVE_PAT | EXI_LOAD_PAT);
	if (entry_not_load_pat_tested)
	...
	wrmsr(MSR_IA32_CR_PAT, host_pat);
	 ...
	 * VM-Exit control is cleared.
	 *
	 * The PAT MSR is loaded with the value from VMCS host PAT MSR field
	 * if the "Load PAT" VM-Exit control is set.
	 */
	...
	enter_guest();
	report(rdmsr(MSR_IA32_CR_PAT) == host_pat, "VMCS host PAT is loaded at VM Exit");
	report(vmcs_read(GUEST_PAT) == 0x0606, "VMCS guest PAT is not saved at VM Exit");

	/*
	 * Case 3:
	 *
	 * The PAT MSR is saved to VMCS during VM-Exit if the "Save PAT" VM-Exit
	 * control is set.
	 *
	 * The PAT MSR is retained if the "Load PAT" VM-Exit control is cleared.
	 */
	 ...
	 * control is set.
	 *
	 * The PAT MSR is loaded with the value from VMCS host PAT MSR field if
	 * the "Load PAT" VM-Exit control is set.
	 */
	...
	if (!entry_not_load_pat_tested) {
		entry_not_load_pat_tested = true;
...
static void vmx_fred_cr0_test_kernel_handler(struct fred_stack_frame *fred_stack_frame)
{
	report(((read_cr4() & X86_CR4_FRED) == X86_CR4_FRED) &&
	       fred_stack_frame->fred_ss.type == 3 &&
	       fred_stack_frame->fred_ss.vector == 13,
	       "#GP on clearing CR0.PG when FRED enabled");
	/* Skip the MOV to CR0 instruction that caused the #GP */
	fred_stack_frame->ip += 3;
}

static void vmx_fred_cr0_test(void)
{
	if (!(ctrl_enter_rev.clr & ENT_LOAD_FRED)) {
		report_skip("Load FRED state entry control is not available");
		return;
	...
	fred_set_kernel_handler(vmx_fred_cr0_test_kernel_handler);
...
static void vmx_nested_fred_cr0_test_guest(void)
{
	ia32_efer = rdmsr(MSR_EFER);
	report((ia32_efer & (EFER_LME | EFER_LMA)) == (EFER_LME | EFER_LMA),
	       "Nested guest in long mode");
...
static void vmx_nested_fred_cr0_test(void)
{
	unsigned long guest_fred_config;
	...
	test_set_guest(vmx_nested_fred_cr0_test_guest);
	enter_guest();
...
struct vmx_test vmx_tests[] = {
	...
	TEST(vmx_fred_cr0_test),
	TEST(vmx_nested_fred_cr4_test),
	TEST(vmx_nested_fred_cr0_test),
	{ NULL, NULL, NULL, NULL },
};
